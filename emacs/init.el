...
;; set up:
;; - webbrowser (eww)
;; - - shrface better rendering
;; - - eww-lnum Vim-like link hints
;; - - elfeed RSS reader that integrates with eww
;; - - engine-mode quick search engines
;; - emails (notmuch)
;; - - org-msg compose HTML mail with org
;; - - notmuch-indicator unread counters
;; - - consult-notmuch fast search
;; - notes app (org-mode)
;; - - org-roam knowledge graph / Zettelkasten
;; - - org-capture instant notes
;; - - org-agenda task management
;; - - org-modern nicer visuals
;; - debugger (dap-ui)
...
(timeload "basic settings"
          ...
          ;; bars
          ;;(menu-bar-mode nil) ;; i like it, if i ever forget a command
          (tool-bar-mode -1)
          (scroll-bar-mode -1)
          (tooltip-mode -1)
          ;; bottom bar
          ...
          (setopt backup-directory-alist         (list (cons "."  (locate-user-emacs-file "backups"))))
          ;;(setopt auto-save-file-name-transforms (list (list ".*" (locate-user-emacs-file "auto-save/") t)))
          ;;(native-compile-async "~/.config/emacs/elpa" t)
...
(custom-set-variables
 ...
 '(package-selected-packages
   '(anki-mode avy-embark-collect cape consult corfu crdt dap-mode dash-functional
               dictionary diff-hl docker editorconfig elisp-autofmt
               elixir-ts-mode embark embark-consult embrace flycheck
               graphviz-dot-mode gruvbox-theme heex-ts-mode
               ...
               yasnippet-snippets)))
;; org-mode: https://abode.karthinks.com/org-latex-preview/ -> https://code.tecosaur.net/tec/org-mode from 
...
(timeload "lsp setup and completion"
          (use-package lsp-mode
            :hook
            ((c-ts-mode c++-ts-mode java-mode haskell-mode python-mode python-ts-mode) . lsp-deferred)
            :commands
            (lsp lsp-deferred)
            :custom
...
(timeload "keybinds"
          ...
          (defvar-keymap custom/yank-mappings
            ...
            "j" (kmacro "C-a C-<SPC> C-n M-w C-y")
            "k" (kmacro "C-e C-<SPC> C-p M-w C-y")
            )
          (keymap-global-set "C-c C-y" custom/yank-mappings)
